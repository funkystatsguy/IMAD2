#' Generate usable binary data set from a
#'
...
#' @param pclin A vector that takes the minimum clinically significant success probabilities of each group
...
#' @export
#'
#' @examples IMAD_bin_dataset
IMAD_bin_dataset <- function(treatment, values, data, burnin = 10, pclin, seed = NA, trtlvls = c("A", "B"), weight = 0.5, alpha = 0.05, power = 0.8){
  ...
  if(length(trtvec) >= (2 * burnin)){
    ...
    if (length(trtvec) > (2 * burnin)){
      for(i in (2 * burnin):(length(trtvec) - 1)){
        ...
        nextval <- IMAD_next_val_bin(newtrtvec1, newtrtvec2, trtlvls, pclin)
    ...
    if(sum(veca == totveca) == length(trtvec) & sum(vecb == totvecb) == length(trtvec)){
      ...
      nextval <- IMAD_next_val_bin(trtvec1, trtvec2, trtlvls, pclin)
